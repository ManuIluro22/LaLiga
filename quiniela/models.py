...
class QuinielaModel:
    """
    A class responsible for handling the training and prediction for Quiniela outcomes.

    This model uses a Gradient Boosting Classifier, and it scales and balance the data with SMOTE. 
    It provides methods to train, predict, and save/load model instances.

    Attributes:
        model (GradientBoostingClassifier): Model used for training and prediction.
        scaler (StandardScaler): StandardScaler instance for feature scaling.    
    """    

    def __init__(self):
        """
        Initialize the model to a Gradient Boosting Classifier with the best found 
        parameters and the scaler to a StandarScaler.

        """
        self.model = GradientBoostingClassifier(learning_rate=0.01, n_estimators=100)
    ...
    def predict(self, predict_data):
        ...
        Args:
            predict_data (pd.DataFrame): Data containing features for prediction.
    ...
    def _extract_features_and_target(self, data):
        ...
        X = final_df.df.drop(columns=['season', 'division', 'home_team', 'away_team', 'result'])
        return X, y
        

    def _scale_data(self, X, train=True):
        """
        Scale features using the StandardScaler. Fits the scaler in the training process

        Args:
            X (pd.DataFrame): Data to be scaled.
            train (boolean, optional): Flag to fit or not the scaler. By default is set 
            to True.
    ...
    def _prepare_features(self, data):
        ...
        """
        X = data.drop(columns=['season', 'division', 'home_team', 'away_team', 'result'])

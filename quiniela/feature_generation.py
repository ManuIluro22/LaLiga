...
class FeatureGenerator:
    ...
    def apply_features(self, quiniela_df, selected_features=[]):
        ...
        Args:
            ...
            selected_features (list, optional): A list of feature names to be generated (avg_goals, last_n_matchdays,
            rank_n_last_seasons and versus_features). If empty, all features will be generated.
        """
        if len(selected_features) == 0 or 'avg_goals' in selected_features:
            self._avg_goals(quiniela_df)
        if len(selected_features) == 0 or 'last_5_matchdays' in selected_features:
            self._last_5_matchdays(quiniela_df)
        if len(selected_features) == 0 or 'rank_5_last_seasons' in selected_features:
    ...
    def _last_5_ranks(self,quiniela_df):
        """
        Computes the average ranking of each team over the last 5 seasons and merges it with the match standings.
    ...
    def _last_5_matchdays(self, quiniela_df):
        """
        Generates results from the last 5 matchdays for each team and calculates average points.
    ...
    def _versus_features(self,matchday_df):
        ...
        matchday_df.df["dif_last_5_points"] = matchday_df.df[f"home_avg_points_last_5"] - matchday_df.df[f"away_avg_points_last_5"]
        ...
        matchday_df.df["dif_previous_5_ranks"] = matchday_df.df[f"home_rank_5_last_seasons"] - matchday_df.df[f"away_rank_5_last_seasons"]
